...
LAST_PHOTO_TIME = 0  # 保存上次拍照时间
PHOTO_ID = 0  # 拍照的ID
PHOTO_ID_MAX = 3  # 照片最大ID
...
def snapshot():
    ...
    if ((time.time() - LAST_PHOTO_TIME) > DELAY_TIME):
        ...
        PHOTO_ID += 1;
        if PHOTO_ID > PHOTO_ID_MAX:
            # 照片ID到上限则归零
            PHOTO_ID = 0
        s = 'echo \'%d\' > /tmp/webcom' % PHOTO_ID
        subprocess.Popen(s, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    if PHOTO_ID == PHOTO_ID_MAX:
        # 实际发送前一张图片ID
        jpgid = 0
    else:
        jpgid = PHOTO_ID + 1
    s = '%d.jpg' % jpgid
    return send_from_directory(r'/tmp/', s)
