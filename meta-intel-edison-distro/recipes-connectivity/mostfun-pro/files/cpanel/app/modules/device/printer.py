...
class gcode(object):
    def __init__(self):
        ...
        self._extruderTargetTemp = 0.0
        self._bedTargetTemp = 0.0
        self.q = list()
        ...
        for command in self.q:
            ...
            if ';' in command:
                if ';Layer count:' in command:
                    self._layerCount = int(re.search(';Layer count: *([0-9]*)',command).group(1))
                elif ';LAYER:' in command:
                    self._layer_offset = int(re.search(";LAYER:(-?[0-9]*)", command).group(1))
                    print self._layer_offset
                    return
...
class mostfun(object):
    def __init__(self, port=SERIALPORT, baud=SERIALBAUD):
        # ready\printing\pause\waiting\error\finished
        self.state = 'ready'
        self._filePath = ''
        ...
        tick = threading.Thread(target=self.clock)
        tick.daemon = True
        tick.start()
    ...
    def resumeTask(self):
        ...
        if self.state == 'pause':
            # self.cleanNozzle()
            print "resunmeeeeeeeeeeeeeeeeeeeeee"
            self.lock()
            tempF = ''
            tempX = ''
            tempY = ''
